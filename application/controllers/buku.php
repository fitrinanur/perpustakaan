...
class Buku extends CI_Controller
{
  ...
  function __construct() {
    parent::__construct();
    //sudah diload di autoload
    //$this->load->library(array('template','pagination','form_validation','upload'));
    $this->load->model('m_buku');
    //ak comment dl , soale blm ada menu login
  ...
  function index($offset = 0, $order_column = 'id', $order_type = 'asc') {
    ...
    //load data
    $data = array();
    $data['buku'] = $this->m_buku->all($this->limit, $offset, $order_column,$order_type)->result();
    ...
    $this->template->load("template_main","buku/index", $data);
  ...
  function add() {
    ...
    if ($this->form_validation->run() == true) {
      ...
      if ($cek->num_rows() > 0) {
        ...
        $info = array('id_lokasi' => $this->input->post('id_lokasi'), 'judul' => $this->input->post('judul'), 'pengarang' => $this->input->post('pengarang'), 'kuantitas' => $this->input->post('kuantitas'));
